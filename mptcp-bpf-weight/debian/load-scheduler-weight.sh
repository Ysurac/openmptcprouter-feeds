#!/bin/sh
# Loads/pins (or unloads/unpins) the bpf_weight MPTCP struct_ops scheduler
...
BPF_OBJ="/usr/share/bpf/scheduler/mptcp_bpf_weight.o"
PIN_DIR="/sys/fs/bpf/mptcp"
# Matches the struct_ops variable name in mptcp_bpf_weight.c ("struct
# mptcp_sched_ops weight = {...}"), i.e. what gets pinned at
# $PIN_DIR/weight -- NOT the same as SCHED_NAME below (that struct's
# .name field, used only for net.mptcp.scheduler).
PIN_NAME="weight"
SCHED_NAME="bpf_weight"
...
start)
	...
	;;
stop)
	# Switch away from bpf_weight *before* unregistering it, otherwise

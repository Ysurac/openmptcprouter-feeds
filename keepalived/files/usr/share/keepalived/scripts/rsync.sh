...
ha_sync_send() {
	...
	# Skip the whole SSH/rsync round while nothing the receiver actually
	# applies has changed since the last successful sync: the per-minute
	# cron otherwise floods the receiver's log with dropbear and sudo
	# entries. The watch set is the union of the peer's sync_list and the
	# files the hotplug scripts register with add_sync_file (the only
	# files the receiver installs); the full transfer list below is left
	# untouched. Broad, constantly-rewritten configs that are transferred
	# but never applied on the receiver (e.g. /etc/config/openmptcprouter,
	# /etc/config/network via sysupgrade -l) are intentionally excluded so
	# they no longer trigger a sync every minute. A real check is still
	# forced every 30 minutes to catch receiver-side drift, and any
	# failure removes the stamp so retries happen every minute.
	stamp="/tmp/keepalived/rsync-${cfg}.stamp"
	if [ -f "$stamp" ] && [ -z "$(find "$stamp" -mmin +30 2>/dev/null)" ]; then
		local watch_list
		watch_list="$sync_list $(sed -n 's/^[[:space:]]*add_sync_file[[:space:]]\{1,\}//p' /etc/hotplug.d/keepalived/* 2>/dev/null)"
		# fall back to the full list if no watch set could be derived
		[ -n "$(echo "$watch_list" | tr -d '[:space:]')" ] || watch_list="$sync_list $(sysupgrade -l)"
		need_sync=0
		for sync_file in $watch_list; do
			if [ -e "$sync_file" ] && [ "$sync_file" -nt "$stamp" ]; then
				need_sync=1
				break
			fi
		done
		if [ "$need_sync" = "0" ]; then
			log_debug "no applied file changed since last sync for $cfg"
	...
	if rsync --out-format='%n' --dry-run -a --relative ${files_list} -e "ssh $ssh_options" --rsync-path="sudo rsync" "$ssh_remote":"$sync_dir" > "$changelog"; then
		...
		if [ "${count%% *}" = "0" ]; then
			...
			mkdir -p /tmp/keepalived && touch "$stamp"
			update_last_sync_time "$cfg"
			update_last_sync_status "$cfg" "Up to Date"
			return 0
		fi

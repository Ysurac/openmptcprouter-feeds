...
| **Block LAN and proxy when cut** | When using the cut action, sets LAN input to `DROP` (uniformly blocking all transparent proxies without changing their service state) and rejects forwarded LAN traffic (bypassed flows, the VPN tunnel, any remaining uplink). The router stays reachable from the LAN and LAN to LAN traffic keeps working; everything is restored when the quota clears. |
